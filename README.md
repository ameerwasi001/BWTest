...
wait = 2
```
Setting a variable is as simple as that and we can do the same for setting strings and floats.
### Getting a variable
...
## Conviniensizer
There's a tool in tools folder for name conviniencizer, it takes your python file and wraps ```self.string``` aroun every string and ```self.number``` around every number. You can use it to ease writing BWTest functions in pythoon for instance this,
...
def find_element_by_class(elem_class, index):
  ...
  self.listOrNot(index.value)
```
## Boolean2bool
Everything given by the language is a value class with certain exceptions specifically variables inside global variable memory and what follows is an example of where it might be used
```
